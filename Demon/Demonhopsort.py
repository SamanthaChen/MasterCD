...
class Demonhopsort(object):
    ...
    def execute(self, G, epsilon=0.25, weighted=False, min_community_size=3):
        ...
        for n in self.G.nodes():
            G.node[n]['communities'] = [n]#初始化社团的标号为自己
        ...
        actual = 0
        '修改的地方'
        degress=nx.degree(self.G)
        sortedDegrees=sorted(degress.items(),key=lambda d:d[1],reverse=True)
        visitedSequence=[val[0] for val in sortedDegrees]
        visitedFlag={}
        visitedFlag=visitedFlag.fromkeys(self.G.nodes(),False)
        for ego in visitedSequence:
            if visitedFlag[ego]==False:
                ...
                # print 'ego:',ego,'ego-ego:',community_to_nodes
                '访问标志'
                for node in ego_minus_ego:
                    visitedFlag[node]=True
                visitedFlag[ego]=True
                #merging phase合并阶段
        ...
        if tuple(actual_community) in communities:
            ...
            for test_community in communities.items():
                ...
                if union is not None:
                    ...
                    communities.pop(test_community[0]) ##合并完吧原来的小社团删掉
                    #if not inserted:
                    communities[tuple(sorted(union))] = 0
                    inserted = True
                    break

            '假如并没有合并，将原来社团直接放到结果中'
