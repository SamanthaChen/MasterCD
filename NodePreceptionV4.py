...
def computeCentrilityandSort3(G):
    ...
    for n in G.nodes():
        ...
        for nei in nx.neighbors(G,n):
            ...
            tmpdensity = 0
            if coreDict[nei] != 0:
                tmp = float(maxDegree) / float(degrees[nei])
                tmp = pow(tmp, 2)
                tmpdensity = pow(e, -tmp)
            densityDict[n] += tmpdensity
            ##计算邻居中最小的core
...
def computeCentrilityandSort4(G):
    '''
    中心度计算
    :param G:
    :return:
    '''
    centrality={}
    centrality=centrality.fromkeys(G.nodes(),0.0)
    for v in G.nodes():
        minSim=1.0
        maxSim=0.0
        for w in nx.neighbors(G,v):
            sim=jaccardSim(nx.neighbors(G,v),nx.neighbors(G,w))
            if sim<minSim:
                minSim=sim
            if sim>maxSim:
                maxSim=sim
        nodes=nx.neighbors(G,v)
        nodes.append(v)
        degrees=nx.degree(G,nodes)
        sortedNodes=[val[0] for val in sorted(degrees.items(),key=lambda d:d[1],reverse=True)]
        M=len(sortedNodes)
        v0=sortedNodes[0]
        if minSim>0:
            centrality[v0]=float(degrees[v0])/float(minSim)
        for i in range(2,M):
            vi=sortedNodes[i]
            if maxSim>0:
                centrality[vi]=float(degrees[v0])/float(maxSim)
    '对节点进行排序'
    visitedSequence=[val[0] for val in sorted(centrality.items(),key=lambda d:d[1],reverse=True)]
    return visitedSequence
...
def absoluteMerge(allcommunities, id1, com1, epsilon):
    if len(allcommunities)==0:
        ...
        return allcommunities
    if com1 in allcommunities.values():
        return allcommunities
    else:
...
def runLFR():
    ...
    for size in sizeList:
        print size,'...'
        outpath=inpath+'NPLFR/network_'+str(size)+'.res'
        '读边格式'
        edgeFile = inpath + 'network_'+ str(size)+'.dat'
        G = nx.read_edgelist(edgeFile, nodetype=int)
        sim=0.1
        start=time.clock()
        res = runOrderedNodePreception(G, sim, slpar=0.3)
        end=time.clock()
...
def runReraldata():
    ...
    dataList3=['facebook_ego'+str(val) for val in egoList]

    for dataName in dataList:
        print dataName
        outpath = inPath + 'NP-BGLL/'+dataName+'_res'
        '读边格式'
        edgeFile=inPath+dataName+'_edges'
        G = nx.read_edgelist(edgeFile,nodetype=int)
        '读adj格式'
        # adjFile=inPath+dataName+'_graph'
        # G=nx.read_adjlist(adjFile,nodetype=int)
        '传统的'
        res=runOrderedNodePreception(G,sim,slpar=1.0)
    # #     'demon'
