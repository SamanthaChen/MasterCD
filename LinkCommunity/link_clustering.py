...
"""
changes 2010-08-27:
    * all three attrFiles files now contain the same community id numbers
...
def read_edgelist_weighted(filename,delimiter=None,nodetype=str,weighttype=float):
    ...
    and the attrFiles includes a dict `ij2wij' mapping edge tuple (i,j) to w_ij
    """
    adj = defaultdict(set)
    edges = set()
    ij2wij = {}
...
def write_edge2cid(e2c,filename,delimiter="\t"):
    ...
    # renumber community id's to be sequential, makes attrFiles file human-readable
...
Output: 
  ...
  The attrFiles filename contains the threshold at which the dendrogram
...
"""
    ...
    parser.add_option("-d", "--delimiter", dest="delimiter", default="\t",
                      help="delimiter of input & attrFiles files [default: tab]")
